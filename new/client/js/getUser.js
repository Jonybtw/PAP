  ...
  xhr.onload = function () {
      if (this.status === 200) {
          try {
              ...
              let routesArray = user.data.routes; // Routes as array or string
              if (typeof routesArray === 'string') {
                  routesArray = routesArray.split(',').map(route => route.trim()); // Convert string to array
              }
              ...
              // Populate routes dropdown directly from the routes array
